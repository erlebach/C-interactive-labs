...
7. **Prior-art research pass (queued).** Run the **deep-research** skill to survey current C++
   teaching/visualization tools (anchors: C++ Insights, Python Tutor, Compiler Explorer, Effective
   C++/Modern C++, learncpp/C++ Primer), score each against the two gates — pedagogical fit + static/
   zero-JS/baked expressibility — and produce a **"tool → interaction → component → topic"** mapping
   that becomes a topic backlog beyond the base 15. Filter (see `COURSE_VIA_TOPICS.md` §8b): steal the
   pedagogy and diagram vocabulary, not the runtime; an idea earns a topic only with a learning
   objective AND a baked, color-redundant, AA-compliant rendering. C++ Insights is the highest-leverage
   anchor (maps to classes + templates; static output → directly bakeable). Feeds steps 3–4.
8. **See `COURSE_VIA_TOPICS.md`** (repo root) — the curriculum-from-topic-libraries design note:
   4-layer architecture, per-subject libraries, reusable-spine-vs-diagram insight, the authoring
   workflow (§8a: fake-data YAML prototype → topics), and the prior-art filter (§8b).
