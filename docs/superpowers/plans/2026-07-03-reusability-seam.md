...
- **Deviation from spec §3.B flagged:** the plan routes `glossary` + both Concept renderers through `_prose_box` but leaves legacy `callout_note` (used by the older `basic_ptr`/`function_args` standalone pages) untouched — forcing it through the shared box adds complexity for no gain. Confirm this is acceptable before Task 1.
