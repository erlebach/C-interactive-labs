...
```bash
python -m cpp_labs.yaml_engine.render_page cpp_labs/pointers_refs/layouts/pointers_refs.rail.yaml
...
*Where the machine lives (rarely edited): the engine is `cpp_labs/yaml_engine/render_page.py`
and the component library is `cpp_labs/components.py`. You should not need either to add
